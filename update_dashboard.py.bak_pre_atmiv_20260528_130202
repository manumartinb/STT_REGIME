"""
Build STT REGIME dashboard (v3: 3 senales estrella IV_CONV / VIX / PUT SKEW NIVEL)
...
# VIX expanding (sobre serie diaria VIX completa)
print('[2] VIX expanding ...', flush=True)
...
vix = vix.dropna(subset=['VIX_Close']).sort_values('dia').reset_index(drop=True)
vix['vix_pct'] = expanding_pct(vix['VIX_Close'].values)
df = df.merge(vix[['dia','VIX_Close','vix_pct']], on='dia', how='left')

# PUT SKEW NIVEL: skew_25d_vs50_pct_expanding @ dte160 (col canonica del file)
print('[3] PUT SKEW NIVEL @ dte160 ...', flush=True)
ps = pd.read_csv(PS_PATH, usecols=['trade_date','dte_target','skew_25d_vs50_pct_expanding'])
ps['trade_date'] = pd.to_datetime(ps['trade_date']).dt.normalize()
ps = ps[ps['dte_target']==160][['trade_date','skew_25d_vs50_pct_expanding']].rename(
...
SIGS = {'ivc': ('ivc_pct','IV_CONV'), 'vix': ('vix_pct','VIX'), 'ps': ('ps_pct','PUT SKEW')}
...
data['tbl_vix']=solo_table('vix_pct','VIX')
...
def joint_table(colA,colB,labA,labB):
    ...
    return t
data['tbl_joint_iv']=joint_table('ivc_pct','vix_pct','IVC','VIX')
data['tbl_joint_ip']=joint_table('ivc_pct','ps_pct','IVC','PS')
data['tbl_joint_vp']=joint_table('vix_pct','ps_pct','VIX','PS')
...
data['tbl_combo_iv']=combo_table('ivc_pct','vix_pct','IVC','VIX')
data['tbl_combo_ip']=combo_table('ivc_pct','ps_pct','IVC','PS')
data['tbl_combo_vp']=combo_table('vix_pct','ps_pct','VIX','PS')
...
data['tbl_triple']=[
    ...
    cohort(sub[top['vix']],'TOP20 VIX solo'),
...
def triple_dual(fname):
    ...
    for ax,agg,ttl in [(axes[0],'mean','MEAN'),(axes[1],'median','MEDIAN')]:
        ...
        ax.axhline(0,color='gray',linewidth=0.5,alpha=0.5); ax.grid(alpha=0.2)
        ax.set_xlabel('dia (d001..d030)'); ax.set_ylabel(f'{ttl} PnL acum (pts)')
        ax.set_title(f'STT - Triple IVC x VIX x PS [{ttl}]'); ax.legend(loc='upper left',fontsize=8)
    fig.tight_layout(); fig.savefig(os.path.join(EVDIR,fname),facecolor=COL_BG); plt.close(fig)
    print(f'    [chart] {fname}')

print('[5] charts ...', flush=True)
traj_dual('ivc_pct','IV_CONV','traj_ivc.png')
traj_dual('vix_pct','VIX','traj_vix.png')
traj_dual('ps_pct','PUT SKEW','traj_ps.png')
heat_dual('ivc_pct','vix_pct','IVC','VIX','heat_iv.png')
heat_dual('ivc_pct','ps_pct','IVC','PS','heat_ip.png')
heat_dual('vix_pct','ps_pct','VIX','PS','heat_vp.png')
combo_dual('ivc_pct','vix_pct','IVC','VIX','comp_iv.png')
combo_dual('ivc_pct','ps_pct','IVC','PS','comp_ip.png')
combo_dual('vix_pct','ps_pct','VIX','PS','comp_vp.png')
